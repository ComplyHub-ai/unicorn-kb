...
>
> Lovable did a remix. The `unicorn/` repo is now pointed at a new codebase
...
Ideally, announce the remix to the team so commits to `unicorn/` pause
briefly. Then:

1. `cd ~/code/unicorn && git log --oneline -5` — note the pre-remix HEAD
   SHA. You'll reference it in the audit doc.
2. `cd ~/code/unicorn-kb && git log --oneline -5` — note the KB HEAD SHA
...
```bash
cd ~/code
rm -rf unicorn       # nuke the stale clone
git clone <new-repo-url> unicorn
cd unicorn
...
Open Claude Code at `~/code/` (sees all three repos). Run:

> The unicorn/ repo was just remixed by Lovable. Regenerate
...
>
> Read the actual unicorn/ codebase to populate these. Don't trust any
...
> - Confidence: high
> - Reflects commit: unicorn@<post-remix HEAD SHA>
...
git commit -m "remix: regenerate codebase-state from unicorn@<sha>"
git push
...
> unicorn/ HEAD is <SHA>. Based on the new codebase-state and the pinned
...
- Pre-remix `unicorn/` HEAD SHA
- Post-remix `unicorn/` HEAD SHA
...
- Remix completed, commits to `unicorn/` resume.
...
- **Not pausing team commits.** Devs committing to `unicorn/` mid-remix
  creates merge headaches you didn't need.
- **Committing codebase-state changes to the wrong repo.** They live in
  `unicorn-kb/`, not `unicorn/`. Easy to get wrong when Claude Code is
